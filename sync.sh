...
while true; do  
  rsync -azr --exclude "*.swp" --exclude 'main' --exclude '/oldData' --exclude '/data' --exclude 'dataExtraction' --exclude '__pycache__' --exclude 'people/' --exclude '/output' --exclude '/saved_models' --exclude '/env' --exclude 'stdout-grid' --exclude 'stderr-grid' --exclude 'stdout-filename' --exclude 'stderr-filename' src/ hzwr87@ncc.clients.dur.ac.uk:~/reidSource --delete
